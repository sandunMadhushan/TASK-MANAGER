...
const cargoUpdated = cargoToml.replace(/^version\s*=\s*"[^"]+"/m, `version = "${nextVersion}"`)
if (cargoUpdated === cargoToml) {
  console.error('sync-version-from-tag: could not update version in src-tauri/Cargo.toml')
  process.exit(1)
}
