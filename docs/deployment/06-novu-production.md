...
The app uses Novu for notifications and (optionally) password-reset email. Local `.env` may point at **local Novu** or placeholders; production needs **Novu Cloud** (or your hosted Novu) values in **Render + Vercel** only.
...
3. Note:
   ...
   - **API key** for server triggers → `NOVU_API_KEY` on Render.
...
Defaults in `.env.example` are names like `task-assigned`, `password-reset`, etc. Your Novu workflow **identifier** must match what you set in Render.
...
| Variable | Host |
|----------|------|
| `NOVU_API_KEY`, `NOVU_BACKEND_URL`, workflow IDs, `VITE_NOVU_APPLICATION_IDENTIFIER` (if server reads it) | **Render** (backend) |
...
- App identifier
- Backend URL
- Secrets on Render vs Novu dashboard
